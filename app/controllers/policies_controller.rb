class PoliciesController < ApplicationController
  ...
  def generate
    ...
    open("public/f/Policy_#{@policy.number}_(#{@policy.dba || @policy.name}).pdf", 'wb') do |f|
      f << @pdfForms.to_pdf
    end
  ...
  def viewPDF
    send_file "public/f/Policy_#{@policy.number}_(#{@policy.dba || @policy.name}).pdf", filename: "Policy_#{@policy.number}_(#{@policy.dba || @policy.name}).pdf", disposition: 'inline', format: 'pdf'
  end
