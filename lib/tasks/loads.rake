...
namespace :load do
  ...
  task :forms => :environment do
    Policy.all.each do |p|
      ...
      elsif p.locations.first.loss_coverage.downcase == "special" && !p.property_forms.include?("CP1030(6/07) ")
      ...
      if p.gl.water_gas_tank.downcase == "yes" && !p.gl_forms.include?("PL-GL-WIG(12/13) ")
